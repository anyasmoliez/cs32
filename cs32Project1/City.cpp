...
City::City(int nRows, int nCols)
    : m_rows(nRows), m_cols(nCols), m_player(nullptr), m_nFlatulans(0), m_history(nRows, nCols)//initializes city's history object
{
...
void City::moveFlatulans()
{
    ...
    for (int k = 0; k < m_nFlatulans; k++)
    {
        ...
        if ((rowdiff == 1 && (coldiff == 1 || coldiff == -1)) || (rowdiff == -1 && (coldiff == 1 || coldiff == -1))) {
    ...
    if (isDiagonal) {
        ...
        history().record(m_player->row(), m_player->col());//record player location when bool is true
        isDiagonal = false;
    }
...
History& City::history() {//returns reference to history object
    return m_history;
}